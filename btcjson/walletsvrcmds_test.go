...
func TestWalletSvrCmds(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "getnewaddress optional account",
			newCmd: func() (interface{}, error) {
		...
		{
			name: "getnewaddress optional address type",
			newCmd: func() (interface{}, error) {
				return btcjson.NewCmd("getnewaddress", "", "legacy")
			},
			staticCmd: func() interface{} {
				return btcjson.NewGetNewAddressCmd(nil, &btcjson.ChangeTypeLegacy)
			},
			marshalled: `{"jsonrpc":"1.0","method":"getnewaddress","params":["","legacy"],"id":1}`,
			unmarshalled: &btcjson.GetNewAddressCmd{
				Account:     btcjson.String(""),
				AddressType: &btcjson.ChangeTypeLegacy,
			},
		},
		{
